...
def calc_outputs_evaluation_matrix(self, outputs, ED):
    for ds in set(outputs['meta_data']['data_set']):
        ...
        # print("outputs['meta_data']['kp_3d'][val_idx].shape: ", outputs['meta_data']['kp_3d'][val_idx].shape)   # B, 54, 3
        # print("outputs['meta_data']['kp_3d'].shape: ", outputs['meta_data']['kp_3d'].shape)   # B, 54, 3
        ...
        # print("real_3d.shape: ", real_3d.shape) # B, 24, 3
        # print("predicts.shape: ", predicts.shape)   # B, 24, 3

        if ds not in constants.dataset_nokp3ds:
            if args().calc_PVE_error and ds in constants.PVE_ds:
                # print(outputs['meta_data']['verts'][val_idx].device)    # cuda:0
                # print(outputs['verts'][val_idx].device) # cpu
                # print("gt: ", outputs['meta_data']['verts'][val_idx].shape)    # B, 64, 6890, 3
                # print("out: ", outputs['verts'][val_idx].shape) # B, 6890, 3
                # print("val_idx: ", val_idx)
                # for i in range(len(val_idx)):
                #     print(outputs['meta_data']['verts'][val_idx][i, 0, 0:10, :])  # 두개씩 같음(?)
                # print("out", outputs['verts'][val_idx][0, 0:10, :])
                # for i in range(64):
                #     print(outputs['meta_data']['verts'][val_idx][0, i, 0:10, :])  # 
                # print("#"*20)
                # batch_PVE = torch.norm(outputs['meta_data']['verts'][val_idx].permute(1,0,2,3)-outputs['verts'][val_idx].to(outputs['meta_data']['verts'][val_idx].device), p=2, dim=-1).mean(-1)

                batch_PVE = torch.norm(outputs['meta_data']['verts'][val_idx]-outputs['verts'][val_idx].to(outputs['meta_data']['verts'][val_idx].device), p=2, dim=-1).mean(-1)
                # batch_PVE = torch.norm(outputs['meta_data']['verts'][val_idx]-outputs['verts'][val_idx].to(outputs['meta_data']['verts'][val_idx].device), p=2, dim=-1).mean(-1)
...
def val_result(self, loader_val, evaluation = False, vis_results=False):
    ...
    eval_model = self.model.module.eval()
    # print(eval_model)
    ED = _init_error_dict()
    ...
    for iter_num, meta_data in enumerate(loader_val):
        ...
        ###################################################################################################################################################
        # ind = [19574, 23733, 24105, 23887, 24013]
        # ind = [19574, 23733, 24105]
        # ind = [23887, 24013]
        # ind = [24105]
        # if iter_num not in ind:
        #     continue
        # print("############################# ", iter_num)
        ###################################################################################################################################################
        ...
        #             vis_cfg={'settings': ['save_img'], 'vids': vis_ids, 'save_dir':self.result_img_dir, 'save_name':iter_num}, index=iter_num, kp3ds=kp3d_vis)

        # show_items = ['mesh', 'joint_sampler', 'pj2d', 'classify']
        # # vis_ids = np.arange(1), None  # 3DPW
        # vis_ids = 0    # 3DOH50K
        # self.visualizer.save_rendered_img(outputs, outputs['meta_data'], show_items=show_items,\
        #         vis_cfg={'settings': ['save_img'], 'vids': vis_ids, 'save_dir':self.result_img_dir, 'save_name':iter_num}, index=iter_num, kp3ds=kp3d_vis)
        ...
        if iter_num % (self.val_batch_size*2) == 0:
            ...
            # print("len(outputs['reorganize_idx']): ", len(outputs['reorganize_idx']))
