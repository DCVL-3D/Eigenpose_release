...
class Demo(Base):
    ...
    def test_eval_oh50k(self):
        data_loader = self._create_single_data_loader(dataset='oh50k', train_flag = False, split='test')
        MPJPE, PA_MPJPE, eval_results = val_result(self,loader_val=data_loader, evaluation=True)
    ...
    def test_coco(self, set_name='demo'):
        predicted_results = []
        self.model.eval()
        # kp2d_mapper = constants.joint_mapping(constants.SMPL_ALL_54,constants.OCHuman_19)
        data_loader = self._create_single_data_loader(dataset='coco', train_flag = False)
        vis_dict = {}
        with torch.no_grad():
            for test_iter,meta_data in enumerate(data_loader):
                outputs = self.net_forward(meta_data, mode='eval')
...
def main():
    with ConfigContext(parse_args(sys.argv[1:])):
        ...
        elif args().eval_dataset == 'oh50k_test':
            demo.test_eval_oh50k()
        elif args().eval_dataset == 'coco_demo':
            demo.test_coco(set_name='demo')
        else:
            demo.test_eval()
