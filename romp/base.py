import sys
sys.path.append('/home/dcvl/anaconda3/envs/romp/lib/python3.8/site-packages')
...
class Base(object):
    def __init__(self, **kwargs):
        ...
        if self.save_visualization_on_img:
            ...
            # self.visualizer = Visualizer(resolution=(512,512), result_img_dir=self.result_img_dir, renderer_type='pyrender')
    ...
    def _build_optimizer(self):
        ...
        ############################################################################################################
        if self.fine_tune:
            self.optimizer, self.scaler = load_optimizer(self.model_path, self.optimizer, self.scaler)
        ############################################################################################################
    ...
    def network_forward(self, model, meta_data, cfg_dict):
        ...
        # print("############# outputs['verts']: ", outputs['verts'].shape)   # B, 6890, 3
