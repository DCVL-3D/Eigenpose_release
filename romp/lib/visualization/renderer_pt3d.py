...
class Renderer(nn.Module):
    def __init__(self, resolution=(512,512), perps=True, R=None, T=None, fov=args().FOV, use_gpu=args().gpu!='-1', with_depth=False):
        ...
        # print("########## CalMat K:", self.cameras.K)   # None
        # self.cameras.K = self.cameras.compute_projection_matrix(znear=0., zfar=100.0, max_y=1.0, min_y=-1.0, max_x=1.0, min_x=-1.0, scale_xyz=torch.Tensor([[1., 1., 1.]]))
        # self.cameras.K = self.cameras.compute_projection_matrix(znear=0., zfar=100.0, max_y=1.0, min_y=-1.0, max_x=1.0, min_x=-1.0, scale_xyz=torch.Tensor([[1., 1., 2./100.]]))
        # print("########## CalMat K:", self.cameras.K)   # 
