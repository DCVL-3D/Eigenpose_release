...
from maps_utils.target_generators import HeatmapGenerator, ScaleAwareHeatmapGenerator  ####################
...
def joint_mapping_21(source_format, target_format, template_format, datasetname):
...
class Image_base(Dataset):
    ...
    def get_item_single_frame(self,index):
        ...
        # print("torch.from_numpy(verts_processed).shape: ", torch.from_numpy(verts_processed).shape) # 64, 6890, 3
    ...
    def parse_multiperson_kp2ds(self, full_kps, datasetname):
        bboxes_normalized = _calc_bbox_normed(full_kps)
        if self.train_flag:
            heatmap, AE_joints = self.generate_heatmap_AEmap(full_kps, datasetname)    ##############################
        else:
            heatmap, AE_joints = np.zeros((21, 64, 64)), np.zeros((self.max_person, 21, 2))
    ...
    def process_kp2ds_bboxes(self, full_kps, img_shape=None, is_pose2d=None, datasetname=None):
        ...
        if is_pose2d.sum()>0:
            ...
            # print("full_kp2d: ", full_kp2d)
            # print("len(full_kp2d[0]): ", len(full_kp2d[0]))   # 54
            person_centers_kp2d, full_kp2ds_kp2d, mask_kp2d, bboxes_hw_norm_kp2d, occluded_by_who, heatmap, AE_joints = self.parse_multiperson_kp2ds(full_kp2d, datasetname)
            # print("####################################### ", heatmap.shape)
    ...
    def process_verts(self, verts, root_trans, used_person_inds, augments=None, valid_mask_verts=None, valid_mask_depth=None):
        ...
        # print("verts_processed.shape: ", verts_processed.shape) # 64, 6890, 3
    ...
    def generate_heatmap_AEmap(self, full_kps, datasetname):
        ...
        self.heatmap_mapper = joint_mapping_21(constants.SMPL_ALL_54, constantname, JOINT_21, datasetname) #################################
    ...
    for _,r in enumerate(dataloader):
        ...
        if with_smpl and r['valid_masks'][0,0,4]:
            ...
            if r['valid_masks'][0,0,6]:
                # print("############", verts.shape)  ############ torch.Size([1, 6890, 3])
                # print("############", verts[0].shape)   ############ torch.Size([6890, 3])   
                # print("############", r['valid_masks'].shape)   ############ torch.Size([2, 64, 8])  
                # print("############", r['valid_masks'][0,:,6].shape)    ############ torch.Size([64])        
        ...
        j3ds = r['kp_3d'][0,0]
        # print("############# ", r['kp_3d'].shape)   # 2, 64, 54, 3
        # print("############# ", j3ds.shape)         # 54, 3
        # print("############# ", j3ds[:,:2].shape)   # 54, 2
        image = r['image_org'][0].numpy().astype(np.uint8)[:,:,::-1]
