...
def OCHuman(base_class=default_mode):
    class OCHuman(Base_Classes[base_class]):

        def __init__(self,train_flag=False,split='test',**kwargs):
            super(OCHuman,self).__init__(train_flag)
            self.min_pts_required = 2
            self.split = split
            self.init_coco()
            self.kp2d_mapper = constants.joint_mapping(constants.OCHuman_19,constants.SMPL_ALL_54)
            logging.info('OCHuman 2D keypoint data has been loaded, total {} samples'.format(len(self)))
        
        def init_coco(self):
            self.root = os.path.join(self.data_folder,"OCHuman")
            self.dataset_name = self.split
            self.annots_file_path = os.path.join(self.root,'annots_{}.npz'.format(self.dataset_name))
            # print(self.annots_file_path)
            if not os.path.exists(self.annots_file_path):
                # print(self._get_anno_file_name())
                # self.coco = COCO(self._get_anno_file_name())
                self.coco = COCO("/home/dcvl/MK/ROMP/dataset/OCHuman/ochuman_coco_format_test_range_0.00_1.00.json")
        ...
        def get_image_info(self,index):
            ...
            # print(self.annots[img_name].shape)
            # print(self.kp2d_mapper.shape)
            ...
            img_info = {'imgpath': imgpath, 'image': image, 'kp2ds': kp2ds, 'track_ids': np.arange(len(kp2ds)),\
                    ...
                    'img_size': image.shape[:2], 'ds': 'ochuman'}
        ...
        def _get_anno_file_name(self):
            return os.path.join(self.root,'ochuman_coco_format_{}_range_0.00_1.00.json'.format(self.dataset_name))
...
if __name__ == '__main__':
    dataset = OCHuman()(train_flag=False)
