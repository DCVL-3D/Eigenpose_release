...
class Loss(nn.Module):
    ...
    def _calc_detection_loss(self, outputs, meta_data):
        ...
        ######################################################################################################
        detect_loss_dict.update({'JointMap': 0})
        jointmap_pred = outputs['joint_maps']
        # a=torch.zeros(32, 64, 64)
        # b=torch.ones(32, 64, 64)
        # c=torch.ones(32, 64, 64)*-1
        # print("###################### a: ", focal_loss(a, a)) # 0
        # print("###################### b: ", focal_loss(b, b)) # 0
        # print("###################### c: ", focal_loss(c, c)) # 65.~
        if 'joint_maps' in outputs:
            jointmap_pred = outputs['joint_maps']
            jointmap_gt = meta_data['heatmap'].to(jointmap_pred.device)
            # print("############## jointmap_gt.shape: ", jointmap_gt.shape)
            for i in range(jointmap_gt.shape[0]):
                for j in range(21):
                    if jointmap_gt[i, j, 0, 0] == -1:
                        jointmap_pred[i, j, :, :] = jointmap_gt[i, j, :, :] = 0
            loss_joint = 0.0
            for i in range(21):
                loss_joint += focal_loss(jointmap_pred[:, i, :, :], jointmap_gt[:, i, :, :])
            detect_loss_dict['JointMap'] = loss_joint/21
        ######################################################################################################
    ...
    def _calc_param_loss(self, outputs, meta_data, matched_mask):
        ...
        if 'params' in outputs:
            ...
            if smpl_pose_masks.sum()>0:
                # print("################### meta_data['params'][smpl_pose_masks,3:22*3].shape: ", meta_data['params'][smpl_pose_masks,3:22*3].shape)
