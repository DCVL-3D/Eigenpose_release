...
class ResultParser(nn.Module):
    ...
    def match_params(self, outputs, meta_data, cfg):
        ...
        if 'params_maps' in outputs and 'params_pred' not in outputs:
            outputs['params_pred'] = self.parameter_sampling_(outputs['params_maps'], outputs['coef_maps'], batch_ids, flat_inds, use_transform=True)

        outputs, meta_data = reorganize_data(outputs, meta_data, exclude_keys, gt_keys, batch_ids, person_ids)
        # outputs['centers_pred'] = torch.stack([flat_inds%args().centermap_size, flat_inds//args().centermap_size],1)
        outputs['centers_pred'] = torch.stack([flat_inds%args().centermap_size, torch.div(flat_inds,args().centermap_size, rounding_mode='trunc')],1)
        return outputs, meta_data

######################################################################################################################################
    def parameter_sampling_(self, params_maps, coef_maps, batch_ids, flat_inds, use_transform=True):
        device = params_maps.device
        if use_transform:
            batch, channel = params_maps.shape[:2]
            params_maps = params_maps.view(batch, channel, -1).permute((0, 2, 1)).contiguous()    # 19 = cam3 + global6 + shape10

            batch, channel = coef_maps.shape[:2]
            coef_maps = coef_maps.view(batch, channel, -1).permute((0, 2, 1)).contiguous()    # 63

        params = params_maps[batch_ids,flat_inds].contiguous()
        coef = coef_maps[batch_ids,flat_inds].contiguous()

        # eigenpose fusion
        eigenpose = np.load('eigenpose_matrix.npy')
        eigenpose = torch.from_numpy(eigenpose).to(device)
        coef = coef.transpose(0, 1)
        theta = torch.matmul(eigenpose, coef)
        theta = theta.transpose(0, 1)

        results = torch.cat((params[:, :9], theta[:, :], params[:, -10:]), 1)
