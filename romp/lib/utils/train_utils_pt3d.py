...
def load_model(path, model, prefix = 'module.', drop_prefix='',optimizer=None, **kwargs):
    ...
    if os.path.exists(path):
        ...
        # copy_state_dict(current_model, pretrained_model, prefix = '', drop_prefix=drop_prefix, **kwargs)
    else:
        logging.warning('model {} not exist!'.format(path))
    return model

def load_optimizer(path, optimizer, scaler, **kwargs):
    logging.info('using previous optimizer and scaler: {}'.format(path))
    if os.path.exists(path):
        checkpoint = torch.load(path)
        # current_optimizer = optimizer.state_dict()
        # current_scaler = scaler.state_dict()
        if isinstance(checkpoint, dict):
            if 'optimizer_state_dict' in checkpoint:
                loaded_optimizer = checkpoint['optimizer_state_dict']
            if 'scaler_state_dict' in checkpoint:
                loaded_scaler = checkpoint['scaler_state_dict']
        optimizer.load_state_dict(loaded_optimizer)
        scaler.load_state_dict(loaded_scaler)
        # copy_state_dict(current_model, pretrained_model, prefix = '', drop_prefix=drop_prefix, **kwargs)
    else:
        logging.warning('file {} not exist!'.format(path))
    return optimizer, scaler


def save_single_model(model, optimizer, scaler, path):
    logging.info('saving {}'.format(path))
    checkpoint = {'model_state_dict':model.state_dict(),'optimizer_state_dict':optimizer.state_dict(), 'scaler_state_dict':scaler.state_dict()}
    # torch.save(model.module.state_dict(), path)
    torch.save(checkpoint, path)

def save_model(model, optimizer, scaler, title, parent_folder=None):
    ...
    save_single_model(model, optimizer, scaler, title)
